    ...
    <div class="container-fluid">
        ...
        <form action="{{route($route.'.store')}}" method="post" class="mb-4">
            ...
            <div class="form-group">
                ...
                <!-- Select -->
                <select name="attribute_group_id" class="form-select" data-choices>
                    <option value="">Не входит в группы</option>
                    @foreach($groups as $group)
                        <option value="{{$group->id}}" {{$group->id == old('attribute_group_id') ? 'selected': ''}}>{{$group->name}}</option>
                    @endforeach
                </select>
